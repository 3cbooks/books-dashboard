...
_STOP_PATTERNS = [
    ...
    # 装帧/版本词（这些不是书名一部分，是营销描述）
    r"\s*精装刷边\s*",  r"\s*精装版?\s*",   r"\s*平装版?\s*",
    r"\s*刷边版?\s*",   r"\s*刷边\s*",      r"\s*限量版?\s*",
    r"\s*编码限量\s*",  r"\s*编号版\s*",    r"\s*限定版?\s*",
    r"\s*编码\s*",      # 残留的"编码"
    r"\s*典藏版?\s*",   r"\s*纪念版\s*",    r"\s*豪华版\s*",
    r"\s*礼盒版?\s*",   r"\s*礼盒\s*",      r"\s*印签版\s*",
    r"\s*亲签版?\s*",   r"\s*特签版?\s*",   r"\s*特签\s*",
    r"\s*签名版?\s*",   r"\s*作者签名\s*",
    r"\s*更有[^\s]*\s*",  # "更有特签""更有印签"
    # 销售/物流词
